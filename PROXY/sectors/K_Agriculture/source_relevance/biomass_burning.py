...
from PROXY.sectors.K_Agriculture.tabular.emission_factors import load_residue_ratios
...
_DEFAULT_DM_NPY = "INPUT/Proxy/ProxySpecific/Agriculture/gfed41s_agri_dm_mean.npy"
_DEFAULT_AREA_NPY = "INPUT/Proxy/ProxySpecific/Agriculture/gfed41s_grid_area.npy"
_DEFAULT_LOOKUP = "INPUT/Proxy/ProxySpecific/Agriculture/gfed41s_nuts2_lookup.parquet"
_PREPROCESS_CMD = "python -m Agriculture.preprocess.build_gfed41s_agri_mean"
_GFED_CACHE_KEY = "_agriculture_gfed_arrays"

# Phase 1.5: values sourced from PROXY/config/agriculture/emission_factors.yaml (section residue_ratios).
# (kg dry residue / kg grain yield, EMEP/EEA Guidebook Table 3-2, NFR 4.F LUCAS LC1).
RESIDUE_RATIOS: dict[str, float] = load_residue_ratios()
...
def _load_gfed_inputs(cfg: dict[str, Any], root) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    hit = cfg.get(_GFED_CACHE_KEY)
    ...
    try:
        ...
        raise ImportError(
            "GFED NUTS2 lookup is Parquet: install pyarrow (pip install pyarrow) for pandas.read_parquet."
        ) from exc
    out = (dm_mean, area_m2, lookup)
    cfg[_GFED_CACHE_KEY] = out
    return out
...
def compute_rho_df(extent_df: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    dm_mean, area_m2, lookup = _load_gfed_inputs(cfg, root)
