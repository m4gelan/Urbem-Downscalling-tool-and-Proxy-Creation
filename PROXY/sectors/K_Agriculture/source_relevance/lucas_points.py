...
from PROXY.sectors.K_Agriculture.tabular.run_countries import parse_run_country_codes
...
def get_lucas_ag_points(cfg: dict[str, Any], root: Path) -> pd.DataFrame:
    ...
    if not lucas_rel:
        lucas_rel = "INPUT/Proxy/ProxySpecific/Agriculture/EU_LUCAS_2022.csv"
    lucas_path = resolve_path(root, lucas_rel)

    corine_rel = lb.get("corine_raster") or inputs.get(
        "corine_raster", "INPUT/Proxy/CORINE/U2018_CLC2018_V2020_20u1_100m.tif"
    )
    ...
    if size == 0:
        raise ValueError(
            ...
            "Replace the file or set lucas_build.lucas_data in agriculture defaults / paths "
