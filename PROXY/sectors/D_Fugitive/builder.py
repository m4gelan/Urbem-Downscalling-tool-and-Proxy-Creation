"""
D_Fugitive sector entrypoint: **reference grid** + **merged CEIP/group config** + shared
...
def _log_reference_grid(ref: dict[str, Any]) -> None:
    """Log CRS and dimensions of the CORINE–NUTS reference window (same idea as B_Industry)."""
    crs = ref.get("crs", "?")
    w = int(ref.get("width", 0) or 0)
    h = int(ref.get("height", 0) or 0)
    logger.info(
        "D_Fugitive: reference window crs=%r width=%d height=%d (CORINE + NUTS + pad_m)",
        crs,
        w,
        h,
    )
...
def _log_domain_bbox(ref: dict[str, Any]) -> None:
    ...
    logger.info("D_Fugitive: reference domain WGS84 bbox (W,S,E,N): (%.4f, %.4f, %.4f, %.4f)", w, s, e, n)


def _merge_fugitive_pipeline_cfg(
    root: Path,
    ...
    *,
    ...
    """
    Build the dict consumed by :func:`PROXY.sectors.D_Fugitive.pipeline.run_fugitive_pipeline`.

    * Path keys come from ``path_cfg`` + ``fugitive.yaml`` (see ``fugitive_paths``,
      ``ceip`` overrides).
    ...
    * Duplicates the OSM path under ``osm_fugitive_gpkg`` for backwards-compatible
    ...
    merged = merge_ceip_group_sector_cfg(
        ...
        sector_paths_key="fugitive_paths",
        default_groups_yaml=default_ceip_profile_relpath(
            root, "D_Fugitive", "groups_yaml"
        ),
        osm_key="fugitive",
...
def build(*, path_cfg: dict[str, Any], sector_cfg: dict[str, Any], country: str) -> dict[str, Any]:
    """
    GNFR D fugitive area weights: CEIP group α, OSM + CORINE proxies, population blend,
    **normalize within CAMS cells** per group then per pollutant band.
    ...
    3. **Run** — :func:`run_fugitive_pipeline` in ``pipeline.py`` (delegates to
       :func:`PROXY.sectors._shared.gnfr_groups.run_gnfr_group_pipeline`).

    Parameters
    ----------
    path_cfg
        Resolved ``paths.yaml`` (proxy rasters, CAMS NetCDF, OSM fugitive GPKG, …).
    sector_cfg
        ``fugitive.yaml`` with ``output_path`` injected by ``PROXY.main``.
    country
    ...
    logger.info("D_Fugitive build: root=%s, cli country=%r", root, ctry)
    ...
    logger.info("D_Fugitive: CORINE (reference)=%s", corine_path)
    logger.info("D_Fugitive: NUTS gpkg=%s pad_m=%s", nuts_gpkg, pad_m)
    ...
    fugitive_cfg = _merge_fugitive_pipeline_cfg(
        root,
        path_cfg,
        sector_cfg,
        country=country,
    ...
    paths = fugitive_cfg.get("paths") or {}
    ...
    proxy_log = bool((fugitive_cfg.get("proxy") or {}).get("log_input_stats", True))
    logger.info("D_Fugitive: merged config — output TIF name=%s", output_path.name)
    logger.info("D_Fugitive: CAMS NetCDF = %s", resolve_path(root, paths.get("cams_nc")))
    logger.info("D_Fugitive: population = %s", resolve_path(root, paths.get("population_tif")))
    logger.info("D_Fugitive: OSM fugitive GPKG = %s", resolve_path(root, paths.get("osm_group_gpkg")))
    logger.info("D_Fugitive: CEIP workbook = %s", resolve_path(root, paths.get("ceip_workbook")))
    logger.info("D_Fugitive: CEIP groups YAML = %s", resolve_path(root, paths.get("ceip_groups_yaml")))
    logger.info("D_Fugitive: pollutants (band order) = %s", fugitive_cfg.get("pollutants"))
    logger.info(
        "D_Fugitive: country_iso3_fallback (CEIP / NUTS) = %s; run.show_progress = %s; "
    ...
    if out_resolved != output_path:
        raise RuntimeError(
            f"Fugitive build wrote {out_resolved} but sector output_path is {output_path}"
        )
    logger.info("D_Fugitive build complete: %s", output_path)
