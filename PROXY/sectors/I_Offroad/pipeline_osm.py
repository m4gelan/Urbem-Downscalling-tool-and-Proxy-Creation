"""Pipeline (1A3ei): OSM hydrocarbon pipeline lines, optional facility blend."""
...
def load_pipeline_union(osm_gpkg: Path, _root: Path) -> gpd.GeoDataFrame:
    """Hydrocarbon pipeline ways from ``osm_offroad_pipeline_lines`` (tag rules in ``osm_schema.yaml``)."""
...
def blend_pipeline_with_facilities(
    *,
    ...
    """Optional facilities raster blend (``facilities_tif`` in paths dict)."""
