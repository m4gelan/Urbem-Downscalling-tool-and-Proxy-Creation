...
from PROXY_V2.dataset_loaders.load_hotmaps import (
    load_hotmaps_hdd,
    load_hotmaps_heat_nres,
    load_hotmaps_heat_res,
)
from PROXY_V2.dataset_loaders.load_population import load_population
from PROXY_V2.dataset_loaders.load_waste_rasters import load_ghsl_smod
from PROXY_V2.sector.C_Othercombustion.M_matrix import MODEL_CLASSES
...
def build_x_matrix(
    ...
    *,
    ...
    pop_z = z_score_inside(pop, inside, upper_quantile=0.99, rescale_to_01=True)
    H_res_z = z_score_inside(H_res, inside, upper_quantile=0.99, rescale_to_01=True)
    H_nres_z = z_score_inside(H_nres, inside, upper_quantile=0.99, rescale_to_01=True)
    Hdd_z = z_score_inside(Hdd, inside, upper_quantile=0.99, rescale_to_01=True)
    ...
    for ki, cls in enumerate(MODEL_CLASSES):
        band = X[:, :, ki]
        m = inside & np.isfinite(band)
        if np.any(m):
            log.info(
                f"  {cls}: min={float(band[m].min()):.6g} max={float(band[m].max()):.6g} "
                f"mean={float(band[m].mean()):.6g}"
            )
    return XBuildResult(
        X=X,
        transform=cor_tr,
