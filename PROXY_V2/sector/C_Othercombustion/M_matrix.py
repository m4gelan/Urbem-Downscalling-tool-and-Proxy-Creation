...
from PROXY_V2.dataset_loaders.load_ef_c_othercombustion import (
    ef_kg_per_tj,
    load_emep,
    parse_emep_tables,
)
from PROXY_V2.dataset_loaders.load_c_othercombustion_specific import (
...
def assemble_m_matrix(
    ...
    missing_ef: set[tuple[str, str]] = set()
    for row in gains.rows:
        ...
        for pi, pol in enumerate(pols):
            ...
            if ef <= 0.0:
                missing_ef.add((row.class_name, row.fuel))
            m[pi, ki] += scale * ef
    if missing_ef:
        log.warning(
            f"C_OtherCombustion: no EMEP EF for {len(missing_ef)} (class, fuel) pairs "
            f"(zero EF used); first 10: {sorted(missing_ef)[:10]}"
        )
    return m
