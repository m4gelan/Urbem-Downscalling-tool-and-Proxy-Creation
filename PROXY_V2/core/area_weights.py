...
from PROXY_V2.alpha.Compute_alpha_matrix import AlphaMatrixResult
...
def fuse_stationary_offroad_weights(
    ...
    """
    Per pollutant: W_off = norm(β_F·S_forest + β_R·S_res + β_B·S_commercial),
    W_comb = α_stat·W_stationary + α_off·W_off.
    Returns (stack P×H×W, W_stat, W_off, W_comb dicts keyed by pollutant label).
    """
    ...
    for j, pol in enumerate(pollutant_labels):
        a_stat, a_off, bF, bR, bB = _offroad_branch_betas(alpha_result.alpha[j], gix)
        ...
        W_s = W_stationary[:, :, j].astype(np.float32, copy=False)
