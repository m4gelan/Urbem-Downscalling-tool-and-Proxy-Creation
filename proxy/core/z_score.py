...
def z_score_inside(
    ...
    *,
    ...
    clipped = np.minimum(vals, np.float32(p_q))
    ...
    z_vals = (clipped - np.float32(mu)) / np.float32(sigma)
    ...
    z_max = float((np.float32(p_q) - mu) / sigma)
    denom = z_max - z_min
    if denom <= 0.0:
        out[valid] = 1.0
    else:
        out[valid] = (z_vals - np.float32(z_min)) / np.float32(denom)
