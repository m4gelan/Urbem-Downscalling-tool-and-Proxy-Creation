...
from proxy.alpha.resolve_NFR import SectorGroupResolution, resolve_sector_groups_from_block
from proxy.core import log
from proxy.core.alias import (
    cams_pollutant_var,
    iso3_from_workbook_country_token,
    workbook_pollutant_label,
)
from proxy.dataset_loaders.load_alpha_workbook import read_alpha_workbook
...
def compute_sector_alpha_matrix(
    ...
    *sector_key* selects the block under ``alpha_methods.yaml`` → ``sectors`` (e.g. ``J_Waste``).
    ...
    for j, lab in enumerate(pol_list):
        ...
        methods_arr[j] = int(m)
        if m == 4:
            raise ValueError(f"Alpha method 4 is not implemented in proxy (sector {sector_key!r})")
