...
def compute_eurostat_f_enduse(
    ...
    *,
    ...
    cache_dirs = [str(x) for x in api.get("cache_dirs", ["proxy/cache/eurostat"])]
    write_cache_dir = str(api.get("write_cache_dir", "proxy/cache/eurostat"))
...
def log_eurostat_end_use_debug(result: EurostatEndUseResult, country_profile: dict[str, str]) -> None:
    ...
    if result.commercial_tj:
        ...
        tot = cp + hh
        log.debug("--- nrg_bal_s final energy, other sectors (unit: TJ) ---")
        log.debug(
        ...
        log.debug(f"  total CP+HH: {tot:.3f} TJ")
        log.debug(
    ...
    if result.tj_by_metric:
        ...
        log.debug("--- nrg_d_hhq household end-use (TJ) ---")
        for m in sorted(result.tj_by_metric.keys()):
            tj = result.tj_by_metric[m]
            pct = 100.0 * tj / tj_sum if tj_sum > 0 else 0.0
            log.debug(f"  {m}: TJ={tj:.3f}  ({pct:.1f}% of HH total)")
    if result.f_by_metric:
        log.debug("--- f_enduse share within residential (before bucket rules) ---")
        for m in sorted(result.f_by_metric.keys()):
            log.debug(f"  {m}: {result.f_by_metric[m]:.6g}")
    log.debug("--- f_enduse by bucket (national energy budget share, dimensionless) ---")
    for b in sorted(result.f_enduse_by_bucket.keys()):
        log.debug(f"  {b}: {result.f_enduse_by_bucket[b]:.6g}")
