#!/usr/bin/env python3
"""
Interactive Folium map of CAMS-REG-ANT GNFR D (Fugitive) for Greece.

Area sources (source_type_index == 1) are drawn as grid-cell rectangles; GNFR **D point**
...
map (toggle layers in the control). By default E-PRTR rows are restricted to **fuel
exploitation-related Annex I main activities**: ``1(a)``, ``1(b)``, ``1(d)``, ``1(e)``,
``1(f)``, ``3(a)``, ``3(b)`` (exact ``EPRTRAnnexIMainActivity`` codes), which span **Energy**
(sector 1) and **Mineral industry** (sector 3, mining). Use ``--eprtr-legacy-sector-filter``
for the old behaviour (sector code only, default energy). ``--eprtr-all-sectors`` applies
only with that legacy mode. Use ``--cams-map`` for **CAMS
only** (no E-PRTR). For one country, add ``--eprtr-map`` for the E-PRTR overlay. When
F1_4 is used, the map also loads **F6_1_IED_Installations** (same EPRTR Annex I allowlist
as F1) and **F5_2_LCP_Energy_Emissions** (one point per LCP, MW and stack pollutants in
popups), unless disabled with ``--no-eprtr-f6-ied`` / ``--no-eprtr-f5-lcp``. CAMS point
popups use
...
emission_category_index: D = 4 (1-based), same ordering as GNFR A..L in this project.

Usage (from project root):
  python Fugitive/Auxiliaries/cams_D_greece_map.py
  python Fugitive/Auxiliaries/cams_D_greece_map.py --pollutant nmvoc
  python Fugitive/Auxiliaries/cams_D_greece_map.py --out-map Fugitive/outputs/cams_D_GRC_nmvoc.html
  python Fugitive/Auxiliaries/cams_D_greece_map.py --whole-grid
  python Fugitive/Auxiliaries/cams_D_greece_map.py --all-countries --map-zoom 4
  python Fugitive/Auxiliaries/cams_D_greece_map.py --all-countries --cams-map --map-zoom 4
  python Fugitive/Auxiliaries/cams_D_greece_map.py --country GRC --eprtr-map
  python Fugitive/Auxiliaries/cams_D_greece_map.py --country GRC --map-points-only
...
# GNFR D = index 4 (1-based): A=1, B=2, C=3, D=4, ...
IDX_D_FUGITIVE = 4
...
_EPRTR_CSV_DIR = Path(__file__).resolve().parents[2] / "data" / "E_PRTR" / "eptr_csv"

DEFAULT_EPRTR_FACILITIES_CSV = _EPRTR_CSV_DIR / "F1_4_Air_Releases_Facilities.csv"

DEFAULT_EPRTR_F5_LCP_CSV = _EPRTR_CSV_DIR / "F5_2_LCP_Energy_Emissions.csv"

DEFAULT_EPRTR_F6_IED_CSV = _EPRTR_CSV_DIR / "F6_1_IED_Installations.csv"

DEFAULT_EPRTR_POLLUTANT = "Non-methane volatile organic compounds (NMVOC)"

# Legacy mode: EPRTR_SectorCode 1 — "Energy" only (no mining sector 3).
DEFAULT_EPRTR_SECTOR_CODE = 1.0

# Annex I codes for fuel exploitation / related: refineries, coke, coal products, mining.
# CSV uses compact labels like ``1(a)`` (not ``1.(a)``). Row-level filter; spans sectors 1 and 3.
EPRTR_FUEL_EXPLOITATION_ANNEX_CODES = frozenset(
    {"1(a)", "1(b)", "1(d)", "1(e)", "1(f)", "3(a)", "3(b)"}
)
...
def _point_popup_html(
    ...
    *,
    ...
    lines = [
        ...
        "<b>GNFR D — point</b> <span style='color:#555'>(CAMS-REG)</span><br/>",
...
def load_eprtr_facilities_aggregated(
    ...
    *,
    ...
    bbox: tuple[float, float, float, float] | None,
    annex_allowlist: frozenset[str] | None = None,
) -> pd.DataFrame:
    """
    If ``annex_allowlist`` is non-empty, keep rows whose stripped
    ``EPRTRAnnexIMainActivity`` is in the set (exact match). Sector code is not applied
    in that case so sector 3 (mining) is included with sector 1 (energy).

    If ``annex_allowlist`` is None or empty, use ``sector_code`` when not None
    (EPRTR_SectorCode match).
    """
    cols = [
        "PublicationDate",
        "countryName",
    ...
    use_annex = annex_allowlist is not None and len(annex_allowlist) > 0
    if use_annex:
        act = df["EPRTRAnnexIMainActivity"].map(
            lambda x: str(x).strip() if pd.notna(x) else ""
        )
        df = df[act.isin(annex_allowlist)]
...
def load_f6_ied_installations(
    csv_path: Path,
    *,
    all_countries: bool,
    country_arg: str,
    bbox: tuple[float, float, float, float] | None,
    annex_allowlist: frozenset[str],
) -> pd.DataFrame:
    """
    F6_1_IED_Installations: keep rows whose ``EPRTRAnnexIMainActivity`` is in the same
    fuel-exploitation set as F1_4 (e.g. ``1(a)``, ``3(b)``).
    """
    cols = [
        "PublicationDate",
        "CountryName",
    ...
    df = pd.read_csv(csv_path, usecols=cols, low_memory=False)
    df = df.rename(columns={"CountryName": "countryName"})
    act = df["EPRTRAnnexIMainActivity"].map(
        lambda x: str(x).strip() if pd.notna(x) else ""
    )
    df = df[act.isin(annex_allowlist)]
    df = df[df["InstallationInspireId"].notna()]
    df = df[df["Longitude"].notna() & df["Latitude"].notna()]
    if not all_countries:
        cname = _eprtr_country_for_filter(country_arg)
        df = df[df["countryName"].astype(str).str.strip() == cname]
    if bbox is not None:
        lon0, lat0, lon1, lat1 = bbox
        df = df[
            (df["Longitude"].astype(float) >= lon0)
            & (df["Longitude"].astype(float) <= lon1)
            & (df["Latitude"].astype(float) >= lat0)
            & (df["Latitude"].astype(float) <= lat1)
        ]
    if df.empty:
        return df
    ae = df.groupby("InstallationInspireId")["EPRTRAnnexIMainActivity"].apply(
        _eprtr_annex_join
    )
    ied = df.groupby("InstallationInspireId")["IEDAnnexIMainActivity"].apply(
        _eprtr_annex_join
    )
    g = df.groupby("InstallationInspireId", as_index=False).agg(
        Longitude=("Longitude", "first"),
        Latitude=("Latitude", "first"),
        facilityName=("installationName", "first"),
        city=("City_of_Facility", "first"),
        countryName=("countryName", "first"),
        year_min=("reportingYear", "min"),
        year_max=("reportingYear", "max"),
        pub_max=("PublicationDate", "max"),
        parent_facilityInspireId=("parent_facilityInspireId", "first"),
    )
    g = g.merge(ae.reset_index(name="annex_eprtr"), on="InstallationInspireId")
    g = g.merge(ied.reset_index(name="annex_ied"), on="InstallationInspireId")
    g["marker_weight"] = 1.0
    return g


def load_f5_lcp_plants(
    csv_path: Path,
    *,
    all_countries: bool,
    country_arg: str,
    ...
    """
    F5_2_LCP_Energy_Emissions: one marker per ``LCPInspireId`` (large combustion plant /
    energy release data). Uses rated thermal power (MW) when present; also aggregates
    reported SO2, NOX, DUST (tonnes/year) for popups.
    """
    ...
    rows: list[dict] = []
    for lcp_id, sub in df.groupby("LCPInspireId", sort=False):
        lon = float(pd.to_numeric(sub["Longitude"], errors="coerce").median())
        lat = float(pd.to_numeric(sub["Latitude"], errors="coerce").median())
        if not (math.isfinite(lon) and math.isfinite(lat)):
            continue
        ch = sub.loc[sub["featureType"] == "LCPCharacteristics", "featureValue"]
        mw_max = float(pd.to_numeric(ch, errors="coerce").max())
        if not math.isfinite(mw_max):
            mw_max = 0.0

        def _sum_ft(ft: str) -> float:
            s = sub.loc[sub["featureType"] == ft, "featureValue"]
        ...
        dust_t = _sum_ft("DUST")
        fuels = sorted(
            {str(x) for x in sub.loc[sub["featureType"].isin(fuel_like), "featureType"].unique()}
        )
        nm = sub["installationPartName"].dropna()
        facility_name = str(nm.iloc[0]) if len(nm) else ""
        city = sub["City_Of_Facility"].dropna()
        city_s = str(city.iloc[0]) if len(city) else ""
        ctry = str(sub["countryName"].iloc[0])
        ym = pd.to_numeric(sub["reportingYear"], errors="coerce")
        year_min = int(ym.min()) if ym.notna().any() else None
        year_max = int(ym.max()) if ym.notna().any() else None
        pub = str(sub["PublicationDate"].iloc[-1]) if len(sub) else ""
        marker_w = mw_max if mw_max > 0.0 else max(1.0, so2_t + nox_t + dust_t)
        rows.append(
            {
                "LCPInspireId": lcp_id,
                ...
                "nox_t": nox_t,
                "dust_t": dust_t,
                "fuel_rows": "; ".join(fuels[:12]) + ("; …" if len(fuels) > 12 else ""),
                "year_min": year_min,
                "year_max": year_max,
                "pub_max": pub,
                "marker_weight": float(marker_w),
            }
        )
    return pd.DataFrame(rows)


def _f6_ied_popup_html(row: pd.Series) -> str:
    iid = html.escape(str(row.get("InstallationInspireId", "") or ""))
    pid = html.escape(str(row.get("parent_facilityInspireId", "") or ""))
    name = html.escape(str(row.get("facilityName", "") or ""))
    city = html.escape(str(row.get("city", "") or ""))
    ctry = html.escape(str(row.get("countryName", "") or ""))
    ae = html.escape(str(row.get("annex_eprtr", "") or ""))
    ie = html.escape(str(row.get("annex_ied", "") or ""))
    y0, y1 = row.get("year_min"), row.get("year_max")
    try:
        ys = f"{int(y0)}–{int(y1)}"
    except (TypeError, ValueError):
        ys = f"{y0}–{y1}"
    pub = html.escape(str(row.get("pub_max", "") or ""))
    lon, lat = float(row["Longitude"]), float(row["Latitude"])
    return (
        "<div style='min-width:220px;max-width:460px;font-size:13px;line-height:1.35'>"
        "<b>E-PRTR F6_1</b> <span style='color:#555'>(IED installations)</span><br/>"
        f"<b>Installation</b> {name}<br/>"
        f"<b>City · country</b> {city} — {ctry}<br/>"
        f"<b>EPRTR Annex I</b> {ae}<br/>"
        f"<b>IED Annex I (subset)</b> {ie}<br/>"
        f"<b>Reporting years</b> {html.escape(ys)}"
        f"&nbsp;· <b>Publication (max)</b> {pub}<br/>"
        f"<b>Location</b> lon {lon:.5f}, lat {lat:.5f}<br/>"
        f"<b>InstallationInspireId</b><br/><span style='word-break:break-all'>{iid}</span><br/>"
        f"<b>parent facility</b><br/><span style='word-break:break-all'>{pid}</span>"
        "</div>"
    )


def _f5_lcp_popup_html(row: pd.Series) -> str:
    lid = html.escape(str(row.get("LCPInspireId", "") or ""))
    name = html.escape(str(row.get("facilityName", "") or ""))
    city = html.escape(str(row.get("city", "") or ""))
    ctry = html.escape(str(row.get("countryName", "") or ""))
    mw = row.get("mw_mw", float("nan"))
    so2 = row.get("so2_t", float("nan"))
    nox = row.get("nox_t", float("nan"))
    dust = row.get("dust_t", float("nan"))
    fuels = html.escape(str(row.get("fuel_rows", "") or ""))
    y0, y1 = row.get("year_min"), row.get("year_max")
    try:
        ys = f"{int(y0)}–{int(y1)}"
    except (TypeError, ValueError):
    ...
    return (
        "<div style='min-width:220px;max-width:460px;font-size:13px;line-height:1.35'>"
        "<b>E-PRTR F5_2</b> <span style='color:#555'>(LCP / energy)</span><br/>"
        f"<b>Plant / part</b> {name}<br/>"
        f"<b>City · country</b> {city} — {ctry}<br/>"
        f"<b>Rated thermal input (max row)</b> {_fmt(float(mw))} MW<br/>"
        f"<b>SO2</b> {_fmt(float(so2))} t/yr &nbsp; <b>NOX</b> {_fmt(float(nox))} t/yr &nbsp; "
        f"<b>DUST</b> {_fmt(float(dust))} t/yr<br/>"
        f"<b>Fuel categories reported</b> {fuels}<br/>"
        f"<b>Reporting years</b> {html.escape(ys)}"
        f"&nbsp;· <b>Publication (sample)</b> {pub}<br/>"
        f"<b>Location</b> lon {lon:.5f}, lat {lat:.5f}<br/>"
        f"<b>LCPInspireId</b><br/><span style='word-break:break-all'>{lid}</span>"
        "</div>"
    )
...
def add_eprtr_facility_markers(
    ...
    *,
    ...
    feature_group_name: str = "E-PRTR — AIR (facilities)",
) -> None:
    import folium
    ...
    if agg.empty:
        if skip_if_empty:
            print(
                "E-PRTR F1_4: no facilities after filters "
                "(F5_2/F6_1 layers may still show)."
            )
            return
        raise SystemExit(
    ...
    cmap.add_to(fmap)
    print(
        f"Map: {n} E-PRTR F1_4 facilities (aggregated AIR releases, green/orange markers)."
    )


def add_f6_ied_markers(
    fmap,
    agg: pd.DataFrame,
    *,
    skip_if_empty: bool = True,
    feature_group_name: str = "E-PRTR — F6_1 IED (Annex filter)",
) -> None:
    import folium
    from branca.colormap import LinearColormap
    ...
    if agg.empty:
        if not skip_if_empty:
            raise SystemExit("F6_1 IED: no installation rows after filters.")
        print("E-PRTR F6_1: no IED installations after filters.")
        return
    vmin, vmax = 0.8, 1.2
    colors = ["#542788", "#8073ac", "#b2abd2", "#fdb863", "#e08214"]
    cmap = LinearColormap(
        colors,
        vmin=vmin,
        vmax=vmax,
        caption="F6_1 IED (uniform weight)",
    )
    fg = folium.FeatureGroup(name=feature_group_name, show=True)
    n = 0
    for _, row in agg.iterrows():
        ...
        folium.CircleMarker(
            ...
            fill_color=cmap(1.0),
            fill_opacity=0.85,
            popup=folium.Popup(_f6_ied_popup_html(row), max_width=480),
        ).add_to(fg)
        n += 1
    fg.add_to(fmap)
    cmap.add_to(fmap)
    print(f"Map: {n} E-PRTR F6_1 IED installations (purple markers).")


def add_f5_lcp_markers(
    fmap,
    agg: pd.DataFrame,
    *,
    skip_if_empty: bool = True,
    feature_group_name: str = "E-PRTR — F5_2 LCP (energy)",
) -> None:
    import folium
    from branca.colormap import LinearColormap
    ...
    if agg.empty:
        if not skip_if_empty:
            raise SystemExit("F5_2 LCP: no plants after filters.")
        print("E-PRTR F5_2: no LCP plants after filters.")
        return
    vals = agg["marker_weight"].astype(np.float64).values
    pos = vals[np.isfinite(vals) & (vals > 0)]
    if pos.size:
        vmin = float(np.min(pos))
        vmax = float(np.max(pos))
    ...
    def caption() -> str:
        return "F5_2 LCP: marker weight (MW, or t/yr SO2+NOX+DUST fallback)"

    cmap = LinearColormap(colors, vmin=vmin, vmax=vmax, caption=caption())

    def radius_for(v: float) -> float:
        if not math.isfinite(v) or v <= 0:
            return 4.0
        t = math.sqrt(v / max(vmax, 1e-30))
        return min(20.0, 4.0 + 13.0 * t)

    fg = folium.FeatureGroup(name=feature_group_name, show=True)
    n = 0
    for _, row in agg.iterrows():
        lon, lat = float(row["Longitude"]), float(row["Latitude"])
        v = float(row["marker_weight"])
        color = cmap(v) if math.isfinite(v) and v > 0 else "#888888"
        folium.CircleMarker(
            location=[lat, lon],
            radius=radius_for(v),
            ...
            fill_color=color,
            fill_opacity=0.8,
            popup=folium.Popup(_f5_lcp_popup_html(row), max_width=480),
        ).add_to(fg)
        n += 1
    fg.add_to(fmap)
    cmap.add_to(fmap)
    print(f"Map: {n} E-PRTR F5_2 LCP plants (blue markers).")
...
def _print_table(
    ...
    print("Totals by pollutant: area vs point (GNFR D, same domain)")
    print(
...
def add_cams_d_layers(
    ...
    *,
    ...
    fg_point = folium.FeatureGroup(
        name="GNFR D (Fugitive) — point",
        show=True,
    )

    if not map_points_only:
        fg_area = folium.FeatureGroup(
            name="D Fugitive - area (grid cell)",
            show=True,
        )
    ...
    if map_points_only:
        print(f"Map: {n_drawn_points} GNFR D point sources (area layer omitted).")
...
def build_folium_map(
    ...
    *,
    ...
    eprtr_feature_group_name: str = "E-PRTR — AIR (facilities)",
    eprtr_f6_df: pd.DataFrame | None = None,
    eprtr_f5_df: pd.DataFrame | None = None,
) -> None:
    import folium
    ...
    overlays = _nonempty_overlay_dfs(eprtr_agg, eprtr_f6_df, eprtr_f5_df)
    if map_center_latlon is not None:
        center_lat, center_lon = map_center_latlon
    elif has_cam_pts and overlays:
        cy = float(np.mean(lat[m_pt]))
        cx = float(np.mean(lon[m_pt]))
        oy = float(
            np.mean(
                [
                    float(d["Latitude"].astype(float).mean())
                    for d in overlays
                ]
            )
        )
        ox = float(
            np.mean(
                [
                    float(d["Longitude"].astype(float).mean())
                    for d in overlays
                ]
            )
        )
        center_lat = (cy + oy) / 2.0
        center_lon = (cx + ox) / 2.0
    elif has_cam_pts:
        ...
        center_lat = float(
            np.mean(
                [float(d["Latitude"].astype(float).mean()) for d in overlays]
            )
        )
        center_lon = float(
            np.mean(
                [float(d["Longitude"].astype(float).mean()) for d in overlays]
            )
        )
    else:
    ...
    if eprtr_agg is not None and eprtr_pollutant_label is not None:
        add_eprtr_facility_markers(
            ...
            skip_if_empty=True,
            feature_group_name=eprtr_feature_group_name,
        )
    if eprtr_f6_df is not None and not eprtr_f6_df.empty:
        add_f6_ied_markers(
            fmap,
            eprtr_f6_df,
            skip_if_empty=False,
            feature_group_name="E-PRTR — F6_1 IED (Annex 1a–f, 3a–b)",
        )
    if eprtr_f5_df is not None and not eprtr_f5_df.empty:
        add_f5_lcp_markers(
            fmap,
            eprtr_f5_df,
            skip_if_empty=False,
            feature_group_name="E-PRTR — F5_2 LCP (large combustion)",
        )
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="CAMS GNFR D (Fugitive): Greece map + area/point source counts and totals."
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Ignore --country; use full CAMS domain (still GNFR D only). "
            "If the default E-PRTR CSV exists, the map shows CAMS GNFR D points and "
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="F5_2 LCP Energy Emissions CSV (default: data/E_PRTR/eptr_csv/F5_2_....csv).",
    )
    ap.add_argument(
        "--eprtr-f6-csv",
        type=Path,
        default=None,
        help="F6_1 IED Installations CSV (default: data/E_PRTR/eptr_csv/F6_1_....csv).",
    )
    ap.add_argument(
        "--no-eprtr-f5-lcp",
        action="store_true",
        help="Do not add the F5_2 LCP (large combustion) facility layer.",
    )
    ap.add_argument(
        "--no-eprtr-f6-ied",
        action="store_true",
        help="Do not add the F6_1 IED installation layer (Annex 1a–f, 3a–b).",
    )
    ap.add_argument(
        "--eprtr-pollutant",
        ...
        metavar="TEXT",
        help=(
            "Exact Pollutant column value to filter (default: NMVOC). Annex 3(a)/3(b) mining "
            "sites often report SOx/PM, not NMVOC; use another pollutant to see them on the overlay."
        ),
    )
    ap.add_argument(
        "--eprtr-sector-code",
        ...
        metavar="N",
        help=(
            "EPRTR_SectorCode when using --eprtr-legacy-sector-filter (default: 1 = Energy). "
            "Ignored if --eprtr-all-sectors or if the default Annex activity allowlist is used."
        ),
    )
    ap.add_argument(
        "--eprtr-all-sectors",
        action="store_true",
        help=(
            "With --eprtr-legacy-sector-filter only: do not restrict by EPRTR_SectorCode."
        ),
    )
    ap.add_argument(
        "--eprtr-legacy-sector-filter",
        action="store_true",
        help=(
            "E-PRTR: use sector code only (default energy sector), not the Annex "
            "1(a,b,d,e,f) / 3(a,b) allowlist. Mining (sector 3) is then excluded unless "
            "--eprtr-all-sectors."
        ),
    )
    ap.add_argument(
        "--map-points-only",
        action="store_true",
        help="Map: draw GNFR D point sources only (no area rectangles).",
    )
    ap.add_argument(
        "--map-include-area",
        action="store_true",
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Output HTML (default: Fugitive/outputs/cams_D_<ISO>_pollutant.html)",
    )
    ap.add_argument(
        "--whole-grid",
        action="store_true",
    ...
    args = ap.parse_args()
    eprtr_use_annex_fuel = not bool(args.eprtr_legacy_sector_filter)
    eprtr_annex_allowlist: frozenset[str] | None = (
        EPRTR_FUEL_EXPLOITATION_ANNEX_CODES if eprtr_use_annex_fuel else None
    )
    eprtr_sector_for_load: float | None
    if eprtr_use_annex_fuel:
        eprtr_sector_for_load = None
    else:
    ...
    if args.eprtr_f5_csv is not None:
        eprtr_f5_path = (
            args.eprtr_f5_csv
            if args.eprtr_f5_csv.is_absolute()
            else root / args.eprtr_f5_csv
        )
    else:
        eprtr_f5_path = DEFAULT_EPRTR_F5_LCP_CSV

    if args.eprtr_f6_csv is not None:
        eprtr_f6_path = (
            args.eprtr_f6_csv
            if args.eprtr_f6_csv.is_absolute()
            else root / args.eprtr_f6_csv
        )
    else:
        eprtr_f6_path = DEFAULT_EPRTR_F6_IED_CSV
    ...
    try:
        ...
        sector_d = emis == IDX_D_FUGITIVE
        domain = _build_domain_mask(lon, lat, ci, country_1b, bbox_t)
        base = sector_d & domain
        if int(np.count_nonzero(base)) == 0:
            raise SystemExit("No GNFR D sources match country/bbox filter.")

        print(f"NetCDF: {nc_path.name}")
        print(
            f"Filter: GNFR D (emission_category_index={IDX_D_FUGITIVE}), "
        ...
        if not args.no_map:
            ...
            if use_eprtr_map:
                agg_eprtr = load_eprtr_facilities_aggregated(
                    ...
                    bbox=bbox_t,
                    annex_allowlist=eprtr_annex_allowlist,
                )
                print(
                    f"E-PRTR: {len(agg_eprtr)} facilities (after filter) "
                    f"from {eprtr_csv_path}"
                )
                if eprtr_annex_allowlist:
                    codes = ", ".join(sorted(eprtr_annex_allowlist))
                    print(
                        "  E-PRTR Annex I filter: EPRTRAnnexIMainActivity in "
                        f"{{{codes}}} (energy + mineral industry rows as in F1_4)"
                    )
                elif eprtr_sector_for_load is not None:
                    label = (
                        " (Energy)"
                        if abs(eprtr_sector_for_load - 1.0) < 1e-6
                        else ""
                    )
                    print(
                ...
                if auto_eprtr_map and not args.eprtr_map:
                    print(
                        "  (same Folium map as CAMS GNFR D points; "
                        "--cams-map to omit E-PRTR.)"
                    )

                if not args.no_eprtr_f6_ied:
                    if eprtr_f6_path.is_file():
                        eprtr_f6_df = load_f6_ied_installations(
                            eprtr_f6_path,
                            all_countries=args.all_countries,
                            country_arg=args.country,
                            bbox=bbox_t,
                            annex_allowlist=EPRTR_FUEL_EXPLOITATION_ANNEX_CODES,
                        )
                        print(
                            f"E-PRTR F6_1: {len(eprtr_f6_df)} IED installations "
                            f"(EPRTRAnnexIMainActivity in fuel-exploration set; {eprtr_f6_path.name})"
                        )
                    else:
                        print(
                            f"Note: F6_1 IED CSV not found, skipping layer ({eprtr_f6_path})"
                        )

                if not args.no_eprtr_f5_lcp:
                    if eprtr_f5_path.is_file():
                        eprtr_f5_df = load_f5_lcp_plants(
                            eprtr_f5_path,
                            all_countries=args.all_countries,
                            country_arg=args.country,
                            bbox=bbox_t,
                        ...
                        print(
                            f"E-PRTR F5_2: {len(eprtr_f5_df)} LCP plants "
                            f"({eprtr_f5_path.name})"
                        )
                    else:
                        print(
                            f"Note: F5_2 LCP CSV not found, skipping layer ({eprtr_f5_path})"
                        )
            ...
            if out_map is None:
                ...
                out_map = root / "Fugitive" / "outputs" / f"cams_D_{tag}_{suffix}.html"
            else:
            ...
            eprtr_fg_name = (
                "E-PRTR — Annex 1(a,b,d,e,f) · 3(a,b) (AIR)"
                if eprtr_use_annex_fuel and use_eprtr_map
                else "E-PRTR — AIR (facilities)"
            )
            build_folium_map(
                ds,
                base,
                ...
                eprtr_feature_group_name=eprtr_fg_name,
                eprtr_f6_df=eprtr_f6_df if use_eprtr_map else None,
                eprtr_f5_df=eprtr_f5_df if use_eprtr_map else None,
            )
