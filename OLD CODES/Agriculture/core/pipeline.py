"""Pipeline administration: class extent -> per-source rho -> scores -> weights -> CSV."""
...
from ..config import load_agriculture_config, project_root
from ..source_relevance import agricultural_soils, biomass_burning, enteric_grazing, fertilized_land
from ..source_relevance import livestock_housing as livestock_housing_mod
from ..source_relevance import manure, rice, soil_liming, urea_application
from .class_extent import build_class_extent_long, load_nuts2_filtered
from .io import resolve_path
from .scoring import compute_pollutant_score, merge_rho_lookup
...
def _process_ids_needed(alpha_cfg: dict[str, Any]) -> set[str]:
    """Processes referenced in alpha for rho tables and scores."""
    out: set[str] = set()
    pol = alpha_cfg.get("pollutants") or {}
    for key in pol.keys():
...
def _build_wide_output(long_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = out.merge(rho_wide.reset_index(), on=["NUTS_ID", "CLC_CODE"], how="left")
    return out
...
def apply_runtime_overrides(
    ...
    *,
    ...
    if outputs_override_dir is not None:
        ...
        runtime["paths"]["outputs"]["output_dir"] = str(outputs_override_dir)
...
def run_pipeline_with_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    corine = resolve_path(root, inputs.get("corine_raster", "Input/CORINE/U2018_CLC2018_V2020_20u1.tif"))
    ...
    print(f"Wrote long weights: {long_out}")
    print(f"Wrote wide weights: {wide_out}")
    print(f"Wrote per-pollutant CSVs under: {pollutant_dir}")

    run_cfg = cfg.get("run") or {}
    intermediary = bool(run_cfg.get("intermediary", outputs.get("intermediary", False)))
    if intermediary:
        inter_dir = resolve_path(
            root, outputs.get("intermediary_dir", "Agriculture/results/intermediary")
        )
        inter_dir.mkdir(parents=True, exist_ok=True)
        for pid, rdf in rho_by_process.items():
            fp = inter_dir / f"rho_{pid}.csv"
            rdf.to_csv(fp, index=False, float_format="%.8f", na_rep="")
        print(f"Wrote intermediary rho tables under: {inter_dir}")

    vis = cfg.get("visualization") or {}
    if vis.get("enabled", False):
        from ..analysis.visualization.plot_nuts_clc_maps import run_visualization
