#!/usr/bin/env python3
"""
Interactive Folium map of CAMS-REG-ANT GNFR C (Other stationary combustion) for Greece.
...
emission_category_index: C = 3 (1-based), same ordering as GNFR A..L in this project.

Usage (from project root):
  python Residential/auxiliaries/cams_C_greece_map.py
  python Residential/auxiliaries/cams_C_greece_map.py --pollutant pm2_5
  python Residential/auxiliaries/cams_C_greece_map.py --out-map Residential/outputs/cams_C_GRC_nox.html
...
# GNFR C = index 3 (1-based)
IDX_C_OTHER_STATIONARY = 3
...
def _print_table(
    ...
    print("Totals by pollutant: area vs point (GNFR C, same domain)")
    print(
...
def add_cams_c_layers(
    ...
    *,
    ...
    fg_area = folium.FeatureGroup(
        name="C OtherStationaryComb - area (grid cell)",
        show=True,
    )
    fg_point = folium.FeatureGroup(
        name="C OtherStationaryComb - point",
        show=True,
    )
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(
        description="CAMS GNFR C (Other stationary combustion): Greece map + area/point totals."
    )
    ...
    ap.add_argument(
        "--all-countries",
        action="store_true",
        help="Ignore --country; use full domain (still GNFR C only)",
    )
    ap.add_argument(
        "--bbox",
        type=float,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Output HTML (default: Residential/outputs/cams_C_<ISO>_pollutant.html)",
    )
    ap.add_argument(
        "--map-max-area-cells",
        type=int,
    ...
    try:
        ...
        sector_c = emis == IDX_C_OTHER_STATIONARY
        domain = _build_domain_mask(lon, lat, ci, country_1b, bbox_t)
        base = sector_c & domain
        if int(np.count_nonzero(base)) == 0:
            raise SystemExit("No GNFR C sources match country/bbox filter.")

        print(f"NetCDF: {nc_path.name}")
        print(
            f"Filter: GNFR C (emission_category_index={IDX_C_OTHER_STATIONARY}), "
        ...
        if not args.no_map:
            ...
            if out_map is None:
                out_map = root / "Residential" / "outputs" / f"cams_C_{tag}_{pol}.html"
            else:
